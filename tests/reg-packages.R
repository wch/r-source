...
## checkRd() for the promptPackage()-generated prototype help page
## (with \packageFOO macros) from outside the package dir (cf. PR#18960)
tools::checkRd("myTst/man/myTst-package.Rd", stages = "build")
## failed in R < 4.7.0 with: file './DESCRIPTION' does not exist

## test warnings for missing/autogenerated Rd content
prompt(identity, filename = "myTst/man/identity.Rd")
print(res <- tools::checkRdContents(dir = "myTst"))
## (would have failed as above in R < 4.7.0 due to a bad Rd_macros_package_dir)
stopifnot(exprs = {
...
file.remove("myTst/man/identity.Rd")
