...

## <FIXME>
## Private versions of AM_INIT_AUTOMAKE and AM_MISSING_PROG.
## Necessary to work around a problem in the current release version of
## automake/aclocal in determining missing_dir with zsh (problem with
## CDPATH and pwd; see the docs in the autoconf CVS for more info).
## Note that the current (2001-04-27) automake CVS has a 2-arg only
## version of AM_MISSING_PROG.
...
## gnome.m4
## CHANGES:
## * Replace AC_ARG_WITH(gnome) by our version in configure.in because
##   we need want_gnome=no by default and use LIBS and CPPFLAGS rather
##   than LDFLAGS and CFLAGS.
## * In AC_ARG_WITH(gnome-includes), standardize doc strings and use
##   CPPFLAGS rather than CFLAGS.
## * In AC_ARG_WITH(gnome-libs), standardize doc strings and use LIBS
##   rather than LDFLAGS.
dnl
