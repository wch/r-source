...
double xDevtoOMA1(double x, DevDesc *dd) { return xDevtoNIC(x, dd); }
...
double yDevtoxOMA2(double y, DevDesc *dd) { return yDevtoNIC(y, dd); }
double xDevtoOMA3(double x, DevDesc *dd) { return xDevtoNIC(x, dd); }
...
double yDevtoxOMA4(double y, DevDesc *dd) { return yDevtoNIC(y, dd); }
double xDevtoNFC(double x, DevDesc *dd) 
{ 
...
double xDevtoMAR1(double x, DevDesc *dd) { return xDevtoUsr(x, dd); }
...
double yDevtoxMAR2(double y, DevDesc *dd) { return yDevtoUsr(y, dd); }
double xDevtoMAR3(double x, DevDesc *dd) { return xDevtoUsr(x, dd); }
...
double yDevtoxMAR4(double y, DevDesc *dd) { return yDevtoUsr(y, dd); }
		 ...
		 double cmWidth, double cmHeight, DevDesc *dd)
{
	switch (dd->gp.rspct) {
		case 0:
			...
			break;
		case 1:
			regionsWithRespect(widths, heights,
					   cmWidth, cmHeight, dd);
			break;
		case 2:
			if (tallLayout(cmWidth, cmHeight, dd))
				regionsRespectingWidth(widths, heights,
						       cmWidth, cmHeight, dd);
			else
				regionsRespectingHeight(widths, heights,
							cmWidth, cmHeight, dd);
	}
}
...
static void subRegion(double *left, double *right, double *bottom, double *top,
	       double mincol, double maxcol, double minrow, double maxrow,
...
int NoDevices() { return (R_NumDevices == 1 || R_CurrentDevice == 0); }

int NumDevices() { return R_NumDevices; }

DevDesc* CurrentDevice() { return R_Devices[R_CurrentDevice]; }

DevDesc* GetDevice(int i) { return R_Devices[i]; }
