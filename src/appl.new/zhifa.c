...
int 
F77_SYMBOL(zhifa) (complex * a, int *lda, int *n,
		   int *kpvt, int *info)
{
	complex akm1, bkm1;
	complex denom;
...
L30:
	...
	for (j = imaxp1; j <= i__1; ++j) {
		...
		d__3 = rowmax, d__4 = (d__1 = a[i__2].r, abs(d__1)) + (d__2 = z__1.r, abs(d__2));
	...
	z__1.i = a[i__2].r * -1. + a[i__2].i * 0.;
	d__3 = rowmax,
	d__4 = (d__1 = a[i__1].r, abs(d__1)) + (d__2 = z__1.r, abs(d__2));
	rowmax = max(d__3, d__4);
L50:
...
L100:
	...
	for (jj = imax; jj <= i__1; ++jj) {
		j = k + imax - jj;
		ZCNJG(&z__1, &a[j + k * a_dim1]);
		t.r = z__1.r;
		t.i = z__1.i;
		i__2 = j + k * a_dim1;
		ZCNJG(&z__1, &a[imax + j * a_dim1]);
		a[i__2].r = z__1.r;
		a[i__2].i = z__1.i;
		i__2 = imax + j * a_dim1;
		a[i__2].r = t.r;
...
L140:
	...
	F77_SYMBOL(zswap) (&imax, &a[imax * a_dim1 + 1], &c__1, &a[(k - 1) * a_dim1 + 1], &
			   c__1);
	i__1 = km1;
	for (jj = imax; jj <= i__1; ++jj) {
		j = km1 + imax - jj;
		ZCNJG(&z__1, &a[j + (k - 1) * a_dim1]);
		t.r = z__1.r;
		t.i = z__1.i;
		i__2 = j + (k - 1) * a_dim1;
		ZCNJG(&z__1, &a[imax + j * a_dim1]);
...
L160:
	...
	ZDIV(&z__1, &a[k - 1 + (k - 1) * a_dim1], &z__2);
	akm1.r = z__1.r;
	akm1.i = z__1.i;
	...
	for (jj = 1; jj <= i__1; ++jj) {
		j = km1 - jj;
		ZDIV(&z__1, &a[j + k * a_dim1], &a[k - 1 + k * a_dim1]);
		bk.r = z__1.r;
		bk.i = z__1.i;
		ZCNJG(&z__2, &a[k - 1 + k * a_dim1]);
		ZDIV(&z__1, &a[j + (k - 1) * a_dim1], &z__2);
		bkm1.r = z__1.r;
		bkm1.i = z__1.i;
