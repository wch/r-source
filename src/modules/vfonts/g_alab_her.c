...
/* PAUL MURRELL
   The R graphics engine only has a GLine to draw from (x1,y1) to (x2,y2)
...
/* this is the version of the flabelwidth() method that is specific to the
   ...
   g_flabelwidth () */
static double GVStrWidth (const unsigned char *s, int typeface, int fontindex,
		   int unit, DevDesc *dd)
{
  double label_width;
...
static double GVStrHeight (const unsigned char *s, int typeface, int fontindex,
		    int unit, DevDesc *dd)
{
  double label_height;
  unsigned short *codestring;
...
static void GVText (double x, double y, int unit, char *s, 
...
/* PAUL MURRELL
   ...
   with saving and restoring of dd->gp.*
*/
  ...
  while ((c = (*ptr++)) != '\0')
      ...
      if (c & RAW_HERSHEY_GLYPH)
	{
	  if (line_width_type != 1)
	    {
	      dd->gp.lwd = HERSHEY_UNITS_TO_DEVICE_UNITS (HERSHEY_STROKE_WIDTH);
	      line_width_type = 1;
	    }
		  ...
		  if (glyphnum >= BEGINNING_OF_KANA)
		    {
		      if (line_width_type != 2)
			{
			dd->gp.lwd = HERSHEY_UNITS_TO_DEVICE_UNITS (HERSHEY_ORIENTAL_STROKE_WIDTH);
			  line_width_type = 2;
			}
		    }
		  else
...
{
    R_setVFontRoutines(GVStrWidth, GVStrHeight, GVText);
}
