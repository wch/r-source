...
void RasterTextRotation(char *str, int nstr, int just, int rot)
{
	...
	if (rot<=90){ 
     ...
     SetRect(&destRect, -mapHeight*sin(toRadian(rot)), -newHeight, mapLength*cos(toRadian(rot)) + gcon, 0); //gcon at the end?
	}else if (rot <= 180){
     ...
     SetRect(&destRect, -newWidth, -mapLength*sin(toRadian(180-rot)) - gcon, gcon, mapHeight*cos(toRadian(180-rot)));	
	}else if (rot <=270){
     ...
     SetRect(&destRect, -mapLength*cos(toRadian(rot-180)) - gcon, -gcon, mapHeight*sin(toRadian(rot-180)), newHeight);  
	}else if (rot <360){
     ...
     SetRect(&destRect, -gcon, -mapHeight*cos(toRadian(360-rot)), newWidth, mapLength*sin(toRadian(360-rot))+gcon);
...
// Input rot in degree
// x and y are in normal coordinate
int BackTranX(int x, int y, int rot)
{
