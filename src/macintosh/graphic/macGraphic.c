...
static int Mac_Open(DevDesc *dd, MacDesc *xd, char *dsp, double wid, double hgt)
{
	SInt16 WinIndex;
	if (!SetBaseFont(xd)) {
...
static double Mac_StrWidth(char *str, DevDesc *dd)
{
   ...
   /*                                                                    */
   /* Mac port: Jago Dic 2000 (Stefano M. Iacus). This is still not      */
...
static void Mac_Line(double x1, double y1, double x2, double y2, int coords, DevDesc *dd)
{
	int		xx1, yy1, xx2, yy2;
	short	dx, dy;
	short	absdx, absdy;
...
/* ************************************************************************************************
New_G_History : You need to create a History record for each graphic window.
************************************************************************************************ */
/* I write this routine because I guess I need to deal with resize event. */
/* Now, I recognize that this will be handle by internal R. If after some */
...
/* ************************************************************************************************
startRecord : This routine will be called to start the record of the picture on the window 
              specified in the Mac_Mode routine.
************************************************************************************************ */
...
/* ************************************************************************************************
stopRecord : This routine will be called to end the record of the picture on the window specified
             in the Mac_Mode routine.
************************************************************************************************ */
void stopRecord(WindowPtr window)
{
#ifdef XXX
...
/* ************************************************************************************************
CleanUpWindow : Try to clean up the drawing window
************************************************************************************************ */
void CleanUpWindow(WindowPtr window)
{
#ifdef XXX
...
/* If you call this routine, the default value about TextSize and TextFace of  */
/* the corresponding Mac Devices will be used                                  */
...
/* No useful in Mac, cause there have no way to queue the system.             */
...
/* However, even you using the System font, there will still exist some chance*/
/* that you saw nothing display on the Screen. In this moment, the only thing */
/* we can do is reinitial the GrafPort.                                       */
