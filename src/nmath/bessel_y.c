...
// set in Rmath.h: #define M_bessel_y_max_alpha 1e7
static const double max_alpha_y = M_bessel_y_max_alpha;
...
double bessel_y(double x, double alpha)
{
    ...
    else if (alpha > max_alpha_y) { // NB: same bound in math_2B() [ ../main/arithmetic.c ]
	MATHLIB_WARNING2(_("besselY(x, nu): nu=%g > %g =: max_alpha_y too large for bessel_y() algorithm"),
			alpha, max_alpha_y);
	// FIXME asymptotic formula (w/ potential accuracy loss) is still much better than NaN
	return ML_NAN;
    }
...
double bessel_y_ex(double x, double alpha, double *by)
{
    ...
    else if (alpha > max_alpha_y) { // NB: same bound in math_2b()
	MATHLIB_WARNING2(_("besselY(x, nu): nu=%g > %g =: max_alpha_y, too large for bessel_y() algorithm"),
			alpha, max_alpha_y);
	return ML_NAN;
    }
 ...
 *******************************************************************
 ...
 ----------------------------------------------------------------------*/
    ...
    double b, c, d, e, f, g, h, p, q, r, s, en,
	en1 = 0, ya = 0., ya1 = 0, /* -Wall */
	ex = *x,
	nu = *alpha;
    if (*nb > 0 && 0. <= nu && nu < 1.) { // always the case for valid input
	...
	double xna = trunc(nu + .5);
	int na = (int) xna;
	if (na == 1) {/* <==>  .5 <= nu = *alpha < 1	 <==>  -.5 <= {new nu} < 0 */
	    nu -= xna;
	}
	if (nu == -.5) {
	     ...
	     * ----------------------------------------------------------------------
	     *  Coefficients for Chebyshev polynomial expansion of 1/gamma(1-x), |x| <= .5
	     * --------------------------------------------------------------------------- */
	...
	} else { /* x > thresh_BESS_Y  = 16.  "large X" */
	    /* ------------------------------------------
	     *  Use Campbell's asymptotic scheme.
	     * -----------------------------------------------------------------------------------
	       Mathematical constants --> get more accurate argument reduction for sin() and cos() : */
	    const static double fivpi = 15.707963267948966192 ; // = 5 * pi
	    const static double pim5  =   .70796326794896619231;// = 5 * pi - 15

	    na = 0;
	    double d1 = trunc(ex / fivpi),
	...
	*ncalc = 1;
	...
	// L450:
	for (int i = *ncalc; i < *nb; ++i)
	    by[i] = ML_NEGINF;/* was 0 */

    } else { // error in inputs:  *nb <= 0  or  nu := *alpha  not in  [0, 1)
	by[0] = 0.;
	*ncalc = min0(*nb,0) - 1;
    }
}
