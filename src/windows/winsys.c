...
static void R_FileAssoc(char *szDirName)
{
    ...
    res = RegCreateKeyEx(HKEY_CLASSES_ROOT, ".rmg", 0, "",
...
int R_QueryMemory(char* regname)
{
    ...
    res = RegCreateKeyEx( HKEY_LOCAL_MACHINE, buff2,0, "",
			  REG_OPTION_NON_VOLATILE,
			  KEY_ALL_ACCESS, NULL, &hkey, &disp);
...
void R_SetMemory(int nsize, int vsize)
{
    ...
    if( nsize > 0 ) {
	res = RegCreateKeyEx( HKEY_USERS, "Software\\R\\NSize",0, "",
			      REG_OPTION_NON_VOLATILE,
			      KEY_ALL_ACCESS, NULL, &hkey, &disp);
	if( res == ERROR_SUCCESS ) {
	    if( disp == REG_OPENED_EXISTING_KEY ) {
    ...
    if( vsize > 0 ) {
	res = RegCreateKeyEx( HKEY_USERS, "Software\\R\\VSize",0, "",
			      REG_OPTION_NON_VOLATILE,
			      KEY_ALL_ACCESS, NULL, &hkey, &disp);
	if( res == ERROR_SUCCESS ) {
	    if( disp == REG_OPENED_EXISTING_KEY ) {
	    ...
	    if( vsize > 1000 ) {
		MessageBox(NULL, "Invalid vector heap size ignored", "R Memory",
			   MB_OK);
	    }
	    else {
		if (vsize < rvsize )
