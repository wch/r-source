/* uncmin.f -- translated by f2c (version 19991025).
...
extern int F77_SYMBOL(dtrsl)(double *, int *, int *, double *, int *,
			     int *);
...
mvmltu(int nr, int n, double *a, double *x, double *y)
{
  ...
  for (i = 0, length = n; i < n; --length, ++i) {
    y[i] = F77_SYMBOL(ddot)(&length, &a[i + i * nr], &one, &x[i], &one);
  }
} /* mvmltu */
...
lltslv(int nr, int n, double *a, double *x, double *b)
{
  ...
  if( x != b) Memcpy(x, b, n);
  F77_SYMBOL(dtrsl)(a, &nr, &n, x, &job, &info);
  job = 10;
  F77_SYMBOL(dtrsl)(a, &nr, &n, x, &job, &info);
} /* lltslv */
       ...
       stepmx, double steptl, double *sx)
{
  ...
  *iretcd = 2;
  ...
  if (sln > stepmx) {
    ...
    scl = stepmx / sln;
    F77_SYMBOL(dscal)(&n, &scl, p, &one);
    sln = stepmx;
  }
  slp = F77_SYMBOL(ddot)(&n, g, &one, p, &one);
  rln = 0.0;
  for (i = 0; i < n; ++i) {
    temp1 = fabs(x[i]);
    temp2 = 1.0/sx[i];
       ...
       stepmx)
{
  ...
  *nwtake = 0;
  if (*fstdog) {
    ...
    *eta = (alpha * .8 * alpha / 
	    (-beta * F77_SYMBOL(ddot)(&n, g, &one, p, &one))) + .2;
    for (i = 0; i < n; ++i) {
  ...
  if (*eta * rnwtln <= *dlt) {
	...
	sc[i] = *dlt / *cln * ssd[i] / sx[i];
      ...
      dot1 = F77_SYMBOL(ddot)(&n, v, &one, ssd, &one);
      dot2 = F77_SYMBOL(ddot)(&n, v, &one, v, &one);
  ...
  if (rnwtln <= hi * *dlt) {
    ...
    *phi = rnwtln - *dlt;
	...
	wrk0[i] = sx[i] * sx[i] * p[i];
      ...
      /* 	  solve l*y = (sx**2)*p */
      F77_SYMBOL(dtrsl)(a, &nr, &n, wrk0, &job, &info);
      /* Computing 2nd power */
      temp1 = F77_SYMBOL(dnrm2)(&n, wrk0, &one);
...
/* 	lower triangular part and diagonal */
  ...
  *noupdt = (itncnt == 1);
  ...
  den1 = F77_SYMBOL(ddot)(&n, s, &one, y, &one);
  snorm2 = F77_SYMBOL(dnrm2)(&n, s, &one);
  ynrm2 = F77_SYMBOL(dnrm2)(&n, y, &one);
  if (den1 < sqrt(epsm) * snorm2 * ynrm2) {
    return;
  }

  mvmlts(nr, n, a, s, t);
  den2 = F77_SYMBOL(ddot)(&n, s, &one, t, &one);
  if (*noupdt) {
    /* 	  h <-- [(s+)y/(s+)hs]h */
       ...
       int *noupdt, double *s, double *y, double *u, double *w)
{
  ...
  *noupdt = (itncnt == 1);
  ...
  mvmltu(nr, n, a, s, u);
  den2 = F77_SYMBOL(ddot)(&n, u, &one, u, &one);
