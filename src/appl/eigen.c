		    ...
		    double *scale, int *m, double *zr, double *zi)
{
	...
	for (j = 1; j <= *m; ++j) {
	    s = zr[i + j * z_dim1];
	    zr[i + j * z_dim1] = zr[k + j * z_dim1];
	    zr[k + j * z_dim1] = s;
	    s = zi[i + j * z_dim1];
	    zi[i + j * z_dim1] = zi[k + j * z_dim1];
	    zi[k + j * z_dim1] = s;
	}
...
static void cbal_(int *nm, int *n, double *ar,
	double *ai, int *low, int *igh, double *scale)
{
    /* System generated locals */
    int ar_dim1, ar_offset, ai_dim1, ai_offset;
...
L190:
	...
	for (j = k; j <= l; ++j) {
	  if (j != i) {
	    c = c + fabs(ar[j + i * ar_dim1]) + fabs(ai[j + i * ai_dim1]);
	    r = r + fabs(ar[i + j * ar_dim1]) + fabs(ai[i + j * ai_dim1]);
	  }
	}
/*     .......... guard against zero c or r due to underflow ......... */
	if (c == 0. || r == 0.) {
...
L240:
	...
	tst1 =
	  fabs(hr[l - 1 + (l - 1) * hr_dim1]) +
	  fabs(hi[l - 1 + (l - 1) * hi_dim1]) +
	  fabs(hr[l	+ l * hr_dim1]) +
	  fabs(hi[l	+ l * hi_dim1]);
	tst2 = tst1 + fabs(hr[l + (l - 1) * hr_dim1]);
	if (tst2 == tst1) {
	    goto L300;
	}
    }
...
static void csroot_(double *xr, double *xi, double *yr,
	double *yi)
{
    /* Local variables */
    double s, ti, tr;
...
static void elmhes_(int *nm, int *n, int *low, int *igh,
	double *a, int *int_)
{
    /* System generated locals */
...
/* L120: */
    ...
    s = (d__1 = h[en + na * h_dim1], fabs(d__1)) + (d__2 = h[na + enm2 *
	    h_dim1], fabs(d__2));
    x = s * .75;
    y = x;
    w = s * -.4375 * s;
...
static void tql2_(int *nm, int *n, double *d, double *e, double *z, int *ierr)
{
    ...
    /* Local variables */
    double c, f, g, h;
    double p, r, s, c2, c3, s2;
    double dl1, el1, tst1, tst2;
    int i, j, k, l, m;
    int l1, l2;
    int ii, mml, nn;
