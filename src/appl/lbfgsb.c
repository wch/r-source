...
static void cmprlb(int, int, double *, 
		   ...
		   int *);
static void dcsrch(double *, double *, 
		   double *, double *, double *, double *, 
...
/* ======================= The end of active ============================= */
static void bmv(int m, double *sy, double *wt, int 
...
/*                                 shift old part of WN1. */
		...
		js = m + jy;
		i__2 = m - jy;
		F77_CALL(dcopy)(&i__2, &wn1[jy + 1 + (jy + 1) * wn1_dim1], &c__1, &wn1[
			jy + jy * wn1_dim1], &c__1);
		i__2 = m - jy;
		F77_CALL(dcopy)(&i__2, &wn1[js + 1 + (js + 1) * wn1_dim1], &c__1, &wn1[
			js + js * wn1_dim1], &c__1);
		i__2 = m - 1;
		F77_CALL(dcopy)(&i__2, &wn1[m + 2 + (jy + 1) * wn1_dim1], &c__1, &wn1[
...
/*        upper triangle of (2,2) block of wn. */
	...
	for (js = is; js <= i__2; ++js) {
	    wn[is + js * wn_dim1] += F77_CALL(ddot)(col, &wn[is * wn_dim1 + 1], &c__1, 
...
/*                               Line search is impossible. */
    ...
    dcsrch(f, gd, stp, &c_b275, &c_b276, &c_b277, &c_b9, stpmx, csave, &
	    isave[1], &dsave[1]);
    *xstep = *stp * *dnorm;
...
/*                              move old information */
	i__1 = *col - 1;
	for (j = 1; j <= i__1; ++j) {
	    F77_CALL(dcopy)(&j, &ss[(j + 1) * ss_dim1 + 2], &c__1, &ss[j * ss_dim1 + 1]
		    , &c__1);
	    i__2 = *col - j;
	    F77_CALL(dcopy)(&i__2, &sy[j + 1 + (j + 1) * sy_dim1], &c__1, &sy[j + j * 
		    sy_dim1], &c__1);
/* L50: */
	}
    }
...
/*                                             and the last column of SS: */
    ...
    i__1 = *col - 1;
    for (j = 1; j <= i__1; ++j) {
	sy[*col + j * sy_dim1] = F77_CALL(ddot)(&n, &d__[1], &c__1, &wy[pointr * 
		wy_dim1 + 1], &c__1);
	ss[j + *col * ss_dim1] = F77_CALL(ddot)(&n, &ws[pointr * ws_dim1 + 1], &c__1, &
		d__[1], &c__1);
	pointr = pointr % m + 1;
/* L51: */
    }
