/* dqrsl.f -- translated by f2c (version 19971204).
...
/* Subroutine */ int dqrsl(doublereal *x, integer *ldx, integer *n, integer *
	...
	*info)
{
    ...
    static doublereal t;
    extern /* Subroutine */ int dcopy(integer *, doublereal *, integer *, 
	    doublereal *, integer *), daxpy(integer *, doublereal *, 
...
/*        set up to compute qy or qty. */
    ...
    if (cqy) {
	dcopy(n, &y[1], &c__1, &qy[1], &c__1);
    }
    if (cqty) {
	dcopy(n, &y[1], &c__1, &qty[1], &c__1);
    }
    if (! cqy) {
	goto L70;
    }
...
/*           compute qy. */
	...
	i__2 = *n - j + 1;
	t = -ddot(&i__2, &x[j + j * x_dim1], &c__1, &qy[j], &c__1) / x[j + j 
		* x_dim1];
	i__2 = *n - j + 1;
	daxpy(&i__2, &t, &x[j + j * x_dim1], &c__1, &qy[j], &c__1);
	x[j + j * x_dim1] = temp;
L50:
/* L60: */
	;
...
/*           compute trans(q)*y. */
	...
	x[j + j * x_dim1] = qraux[j];
	i__2 = *n - j + 1;
	t = -ddot(&i__2, &x[j + j * x_dim1], &c__1, &qty[j], &c__1) / x[j + 
		j * x_dim1];
	i__2 = *n - j + 1;
	daxpy(&i__2, &t, &x[j + j * x_dim1], &c__1, &qty[j], &c__1);
	x[j + j * x_dim1] = temp;
L80:
/* L90: */
	;
...
/*        set up to compute b, rsd, or xb. */
    ...
    if (cb) {
	dcopy(k, &qty[1], &c__1, &b[1], &c__1);
    }
    kp1 = *k + 1;
    if (cxb) {
	dcopy(k, &qty[1], &c__1, &xb[1], &c__1);
    }
    if (cr && *k < *n) {
	i__1 = *n - *k;
	dcopy(&i__1, &qty[kp1], &c__1, &rsd[kp1], &c__1);
    }
    if (! cxb || kp1 > *n) {
	goto L120;
    }
...
L150:
	...
	t = -b[j];
	i__2 = j - 1;
	daxpy(&i__2, &t, &x[j * x_dim1 + 1], &c__1, &b[1], &c__1);
L160:
/* L170: */
	;
    }
...
/*           compute rsd or xb as required. */
	...
	i__2 = *n - j + 1;
	t = -ddot(&i__2, &x[j + j * x_dim1], &c__1, &rsd[j], &c__1) / x[j + 
		j * x_dim1];
	i__2 = *n - j + 1;
	daxpy(&i__2, &t, &x[j + j * x_dim1], &c__1, &rsd[j], &c__1);
L200:
	if (! cxb) {
	    goto L210;
	}
	i__2 = *n - j + 1;
	t = -ddot(&i__2, &x[j + j * x_dim1], &c__1, &xb[j], &c__1) / x[j + j 
		* x_dim1];
	i__2 = *n - j + 1;
	daxpy(&i__2, &t, &x[j + j * x_dim1], &c__1, &xb[j], &c__1);
L210:
	x[j + j * x_dim1] = temp;
L220:
/* L230: */
