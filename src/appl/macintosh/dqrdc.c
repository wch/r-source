/* dqrdc.f -- translated by f2c (version 19971204).
...
/* Subroutine */ int dqrdc(doublereal *x, integer *ldx, integer *n, integer *
	p, doublereal *qraux, integer *jpvt, doublereal *work, integer *job)
{
    ...
    extern /* Subroutine */ int dscal(integer *, doublereal *, doublereal *, 
	    integer *), dswap(integer *, doublereal *, integer *, doublereal 
	    *, integer *);
...
/*        according to jpvt. */
	...
	if (j != pl) {
	    dswap(n, &x[pl * x_dim1 + 1], &c__1, &x[j * x_dim1 + 1], &c__1);
	}
	jpvt[j] = jpvt[pl];
	jpvt[pl] = j;
	++pl;
...
/* L20: */
	...
	dswap(n, &x[pu * x_dim1 + 1], &c__1, &x[j * x_dim1 + 1], &c__1);
	jp = jpvt[pu];
	jpvt[pu] = jpvt[j];
	jpvt[j] = jp;
L30:
...
/* L100: */
	...
	dswap(n, &x[l * x_dim1 + 1], &c__1, &x[maxj * x_dim1 + 1], &c__1);
	qraux[maxj] = qraux[l];
	work[maxj] = work[l];
	jp = jpvt[maxj];
	jpvt[maxj] = jpvt[l];
