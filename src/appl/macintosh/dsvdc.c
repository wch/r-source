/* dsvdc.f -- translated by f2c (version 19971204).
...
/* Subroutine */ int dsvdc(doublereal *x, integer *ldx, integer *n, integer *
	...
	info)
{
    ...
    extern /* Subroutine */ int dswap(integer *, doublereal *, integer *, 
	    doublereal *, integer *), drotg(doublereal *, doublereal *, 
    ...
    static integer lm1, mm1, lp1, mp1;
...
/*     unnecessary initializations of l and ls to keep g77 -Wall happy */
...
L220:
	i__2 = *n - l + 1;
	dscal(&i__2, &c_b44, &u[l + l * u_dim1], &c__1);
	u[l + l * u_dim1] += 1.;
	lm1 = l - 1;
	if (lm1 < 1) {
	    goto L240;
...
/*        ...exit */
	...
	acc = (d__1 = test - ztest, abs(d__1)) / (test + 1e-100);
	if (acc > 1e-15) {
	    goto L380;
	}
...
L500:
	if (wantv) {
	    drot(p, &v[k * v_dim1 + 1], &c__1, &v[m * v_dim1 + 1], &c__1, &
		    cs, &sn);
	}
/* L510: */
    }
...
L520:
	...
	e[k] = cs * e[k];
	if (wantu) {
	    drot(n, &u[k * u_dim1 + 1], &c__1, &u[(l - 1) * u_dim1 + 1], &
		    c__1, &cs, &sn);
	}
/* L530: */
    }
...
/*           make the singular value  positive. */
    ...
    s[l] = -s[l];
    if (wantv) {
	dscal(p, &c_b44, &v[l * v_dim1 + 1], &c__1);
    }
L580:
...
/*           ...exit */
    ...
    s[l + 1] = t;
    if (wantv && l < *p) {
	dswap(p, &v[l * v_dim1 + 1], &c__1, &v[(l + 1) * v_dim1 + 1], &c__1);
    }
    if (wantu && l < *n) {
	dswap(n, &u[l * u_dim1 + 1], &c__1, &u[(l + 1) * u_dim1 + 1], &c__1);
    }
    ++l;
    goto L590;
L600:
