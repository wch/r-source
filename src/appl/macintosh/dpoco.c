/* dpoco.f -- translated by f2c (version 19971204).
...
/* Subroutine */ int dpoco(doublereal *a, integer *lda, integer *n, 
	doublereal *rcond, doublereal *z__, integer *info)
{
    ...
    extern /* Subroutine */ int dscal(integer *, doublereal *, doublereal *, 
	    integer *), dpofa(doublereal *, integer *, integer *, integer *);
...
/*     find norm of a using only upper half */
    ...
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	z__[j] = dasum(&j, &a[j * a_dim1 + 1], &c__1);
	jm1 = j - 1;
	if (jm1 < 1) {
	    goto L20;
	}
...
L120:
	...
	t = -z__[k];
	i__2 = k - 1;
	daxpy(&i__2, &t, &a[k * a_dim1 + 1], &c__1, &z__[1], &c__1);
/* L130: */
    }
    s = 1. / dasum(n, &z__[1], &c__1);
    dscal(n, &s, &z__[1], &c__1);
...
/*        solve trans(r)*v = y */
    ...
    for (k = 1; k <= i__1; ++k) {
	i__2 = k - 1;
	z__[k] -= ddot(&i__2, &a[k * a_dim1 + 1], &c__1, &z__[1], &c__1);
	if ((d__1 = z__[k], abs(d__1)) <= a[k + k * a_dim1]) {
	    goto L140;
	}
	s = a[k + k * a_dim1] / (d__1 = z__[k], abs(d__1));
