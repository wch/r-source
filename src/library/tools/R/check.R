                     ...
                     libdir = NULL)
{
    ...
    c(paste("R_LIBS", rlibs, sep = "="),
      ...
      if(WINDOWS) " R_LIBRARY_USER='no_such_dir'" else "R_LIBRARY_USER=''",
      if(WINDOWS) " R_LIBRARY_SITE='no_such_dir'" else "R_LIBRARY_SITE=''")
}
...
.check_packages <- function(args = NULL)
{
    ...
    check_non_ASCII <- function()
    {
        ...
        if (length(out)) {
            if (!is.na(desc["Encoding"])) noteLog()
            else warnLog()
    ...
    check_data <- function()
    {
        ...
        if (!is_base_pkg && R_check_ascii_data && dir.exists("data")) {
           ...
           if (length(out)) {
                ...
                if (length(bad)) warnLog() else noteLog()
