...
void nscor1_(double *s, int *n, int *n2,
...
*/
    ...
    const static double zero = 0., hh = .025;
...
void init_(double *work)
{
/*	Algorithm AS 177.1   Appl. Statist. (1982) Vol. 31, No. 2

	calling our pnorm() instead of
...
void nscor2_(float *s, int *n, int *n2, int *ier)
{
...
*/
    ...
    const float eps[4] = { .419885f,.450536f,.456936f,.468488f };
    const float dl1[4] = { .112063f,.12177f,.239299f,.215159f };
    const float dl2[4] = { .080122f,.111348f,-.211867f,-.115049f };
    const float gam[4] = { .474798f,.469051f,.208597f,.259784f };
    const float lam[4] = { .282765f,.304856f,.407708f,.414093f };
    const float bb = -.283833f;
    const float d__ = -.106136f;
    const float b1 = .5641896f;

    /* System generated locals */
    double d__1, d__2;

    extern double correc_(int *, int *);

    /* Local variables */
    int i, k;
    float e1, e2, l1, ai, an;
...
/*     calculate normal tail areas for first 3 order statistics. */
    ...
    for (i = 1; i <= k; ++i) {
	ai = (float) i;
	e1 = (ai - eps[i - 1]) / (an + gam[i - 1]);
	e2 = pow((double) e1, (double) lam[i - 1]);
	s[i] = e1 + e2 * (dl1[i - 1] + e2 * dl2[i - 1]) / an - correc_(&i, n);
    }
    if (*n2 == k) {
	goto L20;
    }
...
/*     calculate normal areas for other cases. */
	...
	d__2 = (double) l1;
	e2 = pow(d__1, d__2);
	s[i] = e1 + e2 * (dl1[3] + e2 * dl2[3]) / an - correc_(&i, n);
    }
...
double correc_(int *i, int *n)
{
/*      calculates correction for tail area of the i-th largest of n
	order statistics. */

    const float c1[7] = { 9.5f,28.7f,1.9f,0.f,-7.f,-6.2f,-1.6f };
    const float c2[7] = { -6195.f,-9569.f,-6728.f,-17614.f,-8278.f,-3570.f,
	    1075.f };
    const float c3[7] = { 93380.f,175160.f,410400.f,2157600.f,2.376e6f,
	    2.065e6f,2.065e6f };
