...
Currently Sweave runs every code chunk once.  For figure chunks it
then runs the chunk once more for each graphics device selected, so by
default figure chunks are run twice, once like any other chunk and
once to produce the PDF figure.

This can cause problems if figure chunks produce output or have side
effects, so use figure chunks purely to produce the figure.

This will change in \R{} 2.14.0.
