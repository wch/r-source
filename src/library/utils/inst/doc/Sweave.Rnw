...
programs extract documentation and source code. A \texttt{noweb} file
is a simple text file which consists of a sequence of code and
...
figures.  These are of the form \texttt{prefix-label.ext}.
