...
\end{verbatim}
  ...
  Not directly, but package \texttt{SWord} provides similar
...
\end{verbatim}
ensures the default syntax even after loading package \texttt{R2HTML}.
...
\subsection{`Sweave executes figure chunks twice'}

Or variations on this, as in
\url{https://stat.ethz.ch/pipermail/r-help/2011-April/275198.html}.

Currently Sweave runs every code chunk once (unless option
\texttt{eval=FALSE}).  For figure chunks it then runs the chunk once
more for each graphics device selected, so by default figure chunks
are run twice, once like any other chunk and once to produce the PDF
figure.

This can cause problems if figure chunks produce output or have side
effects, so it is good practice to use figure chunks purely to produce
the figure.

Option \verb|figs.only = TRUE| changes this behaviour, so
figure chunks are only run for the selected graphics device(s).  This
