...
RweaveRd <- function()
{
    list(setup = RweaveRdSetup,
         runcode = RweaveRdRuncode,
         writedoc = RweaveRdWritedoc,
         finish = RweaveRdFinish,
         checkopts = RweaveRdOptions)
}

RweaveRdSyntax <- SweaveSyntaxLatex
RweaveRdSyntax$doc <-  "^[[:space:]]*\\\\end\\{atInstall\\}"
RweaveRdSyntax$code <- "^[[:space:]]*\\\\begin\\{atInstall\\}\\{?([^\\}]*)\\}?.*"
RweaveRdSyntax$docexpr = "\\\\atInstall\\{([^\\}]*)\\}"
RweaveRdSyntax$extension <- "\\.[rR]di$"
RweaveRdSyntax$trans <- NULL

RweaveRdSetup <-
    function(file, syntax=RweaveRdSyntax,
             output=NULL, quiet=FALSE, debug=FALSE, echo=TRUE,
             ...
             pdf=FALSE, eps=FALSE)
{
    if(is.null(output)) {
        prefix.string <- basename(sub(syntax$extension, "", file))
        output <- paste(prefix.string, "Rd", sep=".")
    } else prefix.string <- basename(sub("\\.Rd$", "", output))
    ...
    ## to be on the safe side: see if defaults pass the check
    options <- RweaveRdOptions(options)
...
makeRweaveRdCodeRunner <- function(evalFunc=RweaveEvalWithOpt)
{
    ## Return a function suitable as the 'runcode' element
    ## of an Sweave driver.  evalFunc will be used for the
    ## actual evaluation of chunk code.
    RweaveRdRuncode <- function(object, chunk, options)
      {
          if(!(options$engine %in% c("R", "S"))){
              return(object)
          }
          ...
          if(options$split){
              ...
              chunkout <- object$chunkout[chunkprefix][[1]]
              if(is.null(chunkout)){
                  chunkout <- file(paste(chunkprefix, "Rd", sep="."), "w")
                  if(!is.null(options$label))
                    object$chunkout[[chunkprefix]] <- chunkout
              }
          }
...
RweaveRdRuncode <- makeRweaveRdCodeRunner()

RweaveRdWritedoc <- function(object, chunk)
{
    linesout <- attr(chunk, "srclines")
...
RweaveRdFinish <- function(object, error=FALSE)
{
    outputname <- summary(object$output)$description
    inputname <- object$srcfile$filename
    if(!object$quiet && !error)
        cat("\n",
            gettextf("You can now run R CMD Rd2dvi on '%s'", outputname),
            "\n", sep = "")
    close(object$output)
    if(length(object$chunkout) > 0)
        for(con in object$chunkout) close(con)
    if (object$haveconcordance) {
    	# This output format is subject to change.  Currently it contains
    	# three parts, separated by colons:
    	# 1.  The output .Rd filename
    	# 2.  The input .Rdw filename
    	# 3.  The input line numbers corresponding to each output line.
    	#     This are compressed using the following simple scheme:
    	#     The first line number, followed by
    	#     a run-length encoded diff of the rest of the line numbers.
