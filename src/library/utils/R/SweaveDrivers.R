    ...
    function(object, chunk, options) {
        ...
        if (options$split) {
            ...
            if (is.null(chunkout)) {
                ...
                if(!grepl(.SweaveValidFilenameRegexp, chunkout))
                    warning("file name1", sQuote(chunkout), " is not portable",
        ...
        if (length(devs)) {
            if(!grepl(.SweaveValidFilenameRegexp, chunkprefix))
                warning("file name2 ", sQuote(chunkprefix), " is not portable",
                        call. = FALSE, domain = NA)
...
RtangleRuncode <-  function(object, chunk, options)
{
    ...
    if (options$split) {
        if(!grepl(.SweaveValidFilenameRegexp, chunkprefix))
            warning("file name3 ", sQuote(chunkprefix), " is not portable",
                    call. = FALSE, domain = NA)
