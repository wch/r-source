...
\begin{itemize}
\item The option \texttt{eval=FALSE} for the second chunk indicates that
  the code should not be evaluated (yet). The code is reused in the third
  chunk (details follow in Section~\ref{sec:chunkrefs}).
\item The third chunk is marked as a figure chunk (\texttt{fig=TRUE}) such
...
options have the form \texttt{\textsl{key}=\textsl{value}}, where \texttt{\textsl{value}} can be
...
\texttt{\textsl{key}=\textsl{value}} items) in the environment variable
...
\texttt{\textsl{prefix}} can be set by the option \texttt{prefix.string},
...
One common problem is the quotes produced by \texttt{sQuote()} and
\texttt{dQuote()}: these will be in UTF-8 when \R{} is run in a UTF-8
