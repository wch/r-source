...
called recursively,\footnote{\code{parallel::mclapply} detects this
  and runs nested calls serially.} generating $2n + n^2$ processes on
a machine estimated to have $n = 16$ CPUs.
...
\code{mclapply} if \code{mc.set.seed = TRUE} (the default for
\code{mclapply}).
