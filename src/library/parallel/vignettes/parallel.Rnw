...
\code{mccollect} and \code{mcparallel}.  (\pkg{multicore} used these
names, but also the versions without the prefix which are too easily
masked: e.g.{} package \pkg{lattice} has a function \code{parallel}.)
