...
On Windows the defaut is to report the number of logical CPUs.  On
...
number of physical cores but on older versions the number of physical
CPU packages.
...
stopCluster(cl)
@
Note that whereas with \code{mclapply} all the packages and objects we
use are automatically available on the workers, this is not in
...
computations: for example we could compute \code{cd4.mle} on the works
(as about) or on the master and send the value to the workers. We
illustrate the latter by the following code
<<>>=
cl <- makeCluster(mc)
cd4.rg <- function(data, mle) MASS::mvrnorm(nrow(data), mle$m, mle$v)
cd4.mle <- list(m = colMeans(cd4), v = var(cd4))
...
\code{clusterApplyLB}.  The disadvantage is that the results would not
be reproducible (which does not matter here).
