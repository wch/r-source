...
\item \emph{Via} forking.  \emph{Fork} is a
  ...
  This can cause havoc.\footnote{Some precautions are taken on Mac OS
    ...
    \code{quartz} device are inhibited.  This information is available
    at C level in the \code{Rboolean} variable
...
processes may themselves by using multiple threads through a
...
users, and they may not in future be exported from the name space.
...
Note the earlier comments about using these functions in a GUI
environment.
...
server---for \code{mclapply} can only run on a single shared-memory
system.  There is fallback to serial use when needed, by setting
\code{mc.cores = 1}.
...
communication may be blocked even when using a single machine, and we
have seen environments in which it is unreliable.  There is no
currently any fallback to serial use, nor could there easily be (as
the workers start with a different \R{} environment).
