...
\item \emph{Via} forking.  \emph{Fork} is a
  ...
  share memory. In this package (and \pkg{multicore}) the initial fork
  sends an \R{} expression to be evaluated to the worker, and the
  master package opens a pipe for reading that is used by the worker
  to return the results.

\item \emph{Via} \code{system2("Rscript")} or similar to launch a new
  process on the current machine.  This then needs a way to
  communicate between master and worker processes, which is usually
  done \emph{via} sockets.
