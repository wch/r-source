...
One case where this is not the case is Windows, where only the number
...
the same seed, either because a workspace was restored or because the
workers were started at the same time: otherwise these get a
