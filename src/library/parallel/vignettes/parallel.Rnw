...
\item Random number streams are allocated to nodes, so if the tasks
  ...
  It would however take only slighly more work to allocate a stream to
  each task.
...
We illustrate an approach using package \pkg{parallel} which is used on
part of the CRAN check farm.  Suppose that there is a function
...
itself.  As the installation of a single package can block many
...
av <- seq(min(n, M) + 1L, M, 1L) # available workers
