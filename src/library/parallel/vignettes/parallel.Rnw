...
\code{mccollect} and \code{mcparallel}, (For back-compatibility,
functions \code{parallel} and \code{collect} are also exported: these
names are easily masked.)

A number of functions from \pkg{multicore} are provided but not
exported from the name space.
...
have seen environments in which it is unreliable.  There is not
