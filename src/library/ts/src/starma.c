...
/*  start of as 182 */
void
forkal(int *ip, int *iq, int *ir, int *
	np, int *ird, int *irz, int *id, int *il, int *n, 
	int *nrbar, double *phi, double *theta, double *delta,
	 double *w, double *y, double *amse, double *a, 
	double *p, double *v, double *resid, double *xnext, 
	double *xrow, double *rbar, double *thetab, double *
	store, int *ifault)
{
    /* Local variables */
    int id2r1, iddr, id2r2;
    double phii, phij;
    int iupd, idrr1, i, j, k, l;
    double sigma, a1;
    int j1, k1;
    double aa;
    int i45, jj, kk, lk, ll;
...
/*     Evaluate likelihood to obtain final Kalman filter conditions */
    ...
    karma(ip, iq, ir, np, &phi[1], theta, &a[1], &p[1], &v[1], &nt, 
	  &w[1], resid, &sumlog, &ssq, &iupd, &del, &nit);
