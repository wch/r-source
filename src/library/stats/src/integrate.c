...
SEXP call_dqags(SEXP args)
{
    ...
    SET_VECTOR_ELT(ans, 0, ScalarReal(result));
    SET_STRING_ELT(ansnames, 1, mkChar("abs.error"));
    SET_VECTOR_ELT(ans, 1, ScalarReal(abserr));
    SET_STRING_ELT(ansnames, 2, mkChar("subdivisions"));
    SET_VECTOR_ELT(ans, 2, ScalarInteger(last));
    SET_STRING_ELT(ansnames, 3, mkChar("ierr"));
    SET_VECTOR_ELT(ans, 3, ScalarInteger(ier));
