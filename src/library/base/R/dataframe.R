    ...
    if(is.character(row.names)){
      if(length(row.names) == length(attr(x, "row.names")))
        attr(x, "row.names") <- row.names
                      ...
                      "for a data frame with", length(attr(x, "row.names")), "rows"))
    }
    x
  }
...
data.frame <-
  function(..., row.names = NULL, check.rows = F, check.names = T)
  {
    if(check.rows && missing(row.names))
      ...
      {
        if(is.null(current) &&
           !any(duplicated(new <- as.character(new))))
          new
        else current
      }
    ...
    nrows <- numeric(n)
    for(i in 1:n) {
      xi <- as.data.frame(x[[i]], optional=T)
      rowsi <- attr(xi, "row.names")
      nnew <- length(xi)
      namesi <- names(xi)
      if(nnew>1) {
        if(!length(namesi)) namesi <- seq(length=nnew)
        if(no.vn[i]) vnames[[i]] <- namesi
        else vnames[[i]] <- paste(vnames[[i]], namesi, sep=".")
      }
      else if(length(namesi)) vnames[[i]] <- namesi
    ...
    value <- unlist(value, recursive=F, use.names=F)
    ...
    if(all(i >= 0) && (nn <- max(i)) > nrows) {
      ...
      if(!char.i) {
        ...
        if(inherits(value, "data.frame") &&
          ...
          new.rows <- attr(value, "row.names")[1:length(
                                                        nrr)]
  ...
  function(x, i, j, value)
  {
    ...
    if(nargs() < 4) {
      ...
      if(is.null(value)) {}
        ...
        if(!inherits(value, "data.frame")) value <-
          as.data.frame(value)
        if(length(value)!=1) stop(paste(
                   "trying to replace one column with", length(value)))
    ...
    if(all(i >= 0) && (nn <- max(i)) > nrows) {
      ...
      if(!n) {
        nrr <- as.character((nrows + 1):nn)
        if(inherits(value, "data.frame") && (nrv <- dim(
                                                        value)[1]) >= length(nrr)) {
          new.rows <- attr(value, "row.names")[1:length(
                                                        nrr)]
    ...
    if(is.character(j)) {
      ...
      if(any(is.na(jseq))) stop(paste("replacing element in non-existent column:", j[is.na(jseq)]))
    }
    else if(is.logical(j) || min(j) < 0)
      jseq <- seq(along = x)[j]
    else {
      jseq <- j
      if(max(jseq) > nvars) stop(paste(
                                       "replacing element in non-existent column:", jseq[jseq>nvars]))
    }
...
rbind.data.frame <-
  function(..., deparse.level = 1)
  {
    ...
    n <- nargs()
    if(!n)
      return(structure(list(), class = "data.frame", row.names = 
                       character()))
    all <- list(...)
    nms <- names(all)
    if(is.null(nms))
    ...
    for(i in 1:n) {
      ...
      if(inherits(xi, "data.frame")) {
        ...
        rows[[i]] <- nrow <- nrow + 1
        rlabs[[i]] <- if(nchar(nmi)) nmi else nrow
      }
    }
    nvar <- length(clabs)
    if(nvar == 0)
