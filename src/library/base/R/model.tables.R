...
model.tables.aov <- function(x, type = "effects", se = FALSE, cterms)
{
  ...
  if(type == "residuals") stop(paste("type", type, "not implemented yet"))
...
model.tables.aovlist <- function(x, type = "effects", se = FALSE, ...)
{
  ...
  if(type == "effects" || type == "means") {
    if(any(duplicated(nms <- names(dn.proj)[names(dn.proj) != "Residuals"]))) {
      ...
      #Elect to use the effects from the lowest stratum: usually expect this 
      #to be highest efficiency
      eff.used <- apply(efficiency, 2, function(x, ind = seq(length(x)))
                        {
                          temp <- (x > 0)
                          if(sum(temp) == 1) temp
...
se.aovlist <- 
  function(object, dn.proj, dn.strata, factors, mf, efficiency, n,
           type = "diff.means", ...)
{
  if(type != "effects")
  ...
  function(proj.cols, mf.cols, prjs, mf, fun = "mean", prt = FALSE, ...)
{
  ...
  for(i in seq(length(tables))) {
    terms <- proj.cols[[i]]
    if(length(terms) == 1) data <- prjs[, terms]
    else data <- prjs[, terms] %*% as.matrix(rep(1, length(terms)))
    ...
    for(i in seq(length(tables))) {
      ...
      if(length(terms) == 1)
        ...
        if(length(strata <- unique(strata.cols[terms])) == 1)
          data <- projections[[strata]][, terms] %*% as.matrix(1/eff[eff.i])
        else {
          mat <- NULL
          for(j in strata) {
...
replications <- function(formula, data = NULL, na.action = na.fail)
{
  ...
  for(i in seq(length = n)) {
    ...
    if(any(nn <- notfactor[select])) {
      warning(paste("non-factors ignored:", paste(names(nn), collapse = ", ")))
      next
    }
    if(length(select) > 0) tble <- tapply(dummy, unclass(data[select]), length)
    nrep <- unique(tble)
    if(length(nrep) > 1) {
      balance <- FALSE
      tble[is.na(tble)] <- 0
...
eff.aovlist <- function(aovlist)
{
  ...
  for(i in names(proj.len))
    ind <-
      rbind(ind, cbind(match(i, s.labs), match(names(proj.len[[i]]), t.labs)))
