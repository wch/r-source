...
R.home <- function(component="home")
{
    ...
    switch(component,
           ...
           "share" = if(nzchar(p <- Sys.getenv("R_SHARE_DIR", names = FALSE))) p
           else file.path(rh, component),
	   "doc" = if(nzchar(p <- Sys.getenv("R_DOC_DIR", names = FALSE))) p
           else file.path(rh, component),
           "include" = if(nzchar(p <- Sys.getenv("R_INCLUDE_DIR", names = FALSE))) p
           else file.path(rh, component),
