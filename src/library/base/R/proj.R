...
proj.lm <- 
function(object, onedf = FALSE, unweighted.scale = FALSE)
{
...
#    attributes(x)[names(value)] <- value not allowed in R
  ...
  factors.aovlist <- function(pnames, tfactor, strata = FALSE, efactor = FALSE)
  {
    ...
    if(!missing(efactor)) {
      ...
      if(!is.na(err <- match(strata, colnames(efactor))))
        ...
        enames <- c(rownames(efactor)[as.logical(efactor %*% rep(1, ncol(efactor)))], "Within")
      if(!is.null(enames))
        tnames <- append(tnames, list(Residuals = enames))
    }
    result <- tnames[match(pnames, names(tnames))]
