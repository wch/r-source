...
function(x, full = TRUE, scale = TRUE, radius = TRUE, labels =
         dimnames(x)[[1]], locations = NULL, xlimit = NULL, ylimit =
         NULL, len = 1, colors = NULL, key.loc = NULL, key.labels =
         NULL, draw.segments = FALSE, draw.axes = FALSE, ...) {
    ...
    if (full)
        angles <- seq(0, 2*pi, length=n.seg+1)[-(n.seg+1)]
    else
        if ( draw.segments )
            angles <- seq(0, pi, length=n.seg+1)[-(n.seg+1)]
        else
            angles <- seq(0, pi, length=n.seg)
    ...
    if ( is.null(xlimit) )
        temp.xlim <- range(loc[,1] + max(x), loc[,1] - max(x))
    else
        temp.xlim <- xlimit
   
    if ( is.null(ylimit) )
        temp.ylim <- range(loc[,2] + max(x), loc[,2] - max(x))
    else
        temp.ylim <- ylimit
    ...
    plot(0, type="n", ..., xlim=temp.xlim, ylim=temp.ylim, xlab="",
         ylab="", asp = 1, axes = draw.axes)
    ...
    if ( draw.segments ) {
        ...
        for ( i in 1:n.loc ) {
            ...
            if (! is.null(labels) )
                if (full)
                    text(loc[i,1], loc[i,2] - max(x), labels[i],
                         cex=0.5, adj=c(0.5,1))
                else
                    text(loc[i,1], loc[i,2] - 0.1 * max(x), labels[i],
                         cex=0.5, adj=c(0.5,1))
        }
            ...
            if (lab.angl[k] <= (90 * deg))
                ...
                text.adj <-
                    c(text.adj, 1 - (0.5 * (lab.angl[k] - (270 * deg)) /
                                     (90 * deg))) 
            text.default(x=label.x.coord[k], y=label.y.coord[k], labels
                         = key.labels[k], cex = 0.5, adj = text.adj) 
        }
