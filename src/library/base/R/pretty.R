                   ...
                   eps.correct = 0)
{
	...
	stop(sQuote("shrink.sml"), " must be numeric > 0")
    if((min.n <- as.integer(min.n)) < 0 || min.n > n)
	stop(sQuote("min.n"), " must be non-negative integer <= n")
    if(!is.numeric(high.u.bias) || high.u.bias < 0)
	stop(sQuote("high.u.bias"), " must be non-negative numeric")
    if(!is.numeric(u5.bias) || u5.bias < 0)
	stop(sQuote("u5.bias"), " must be non-negative numeric")
    if((eps.correct <- as.integer(eps.correct)) < 0 || eps.correct > 2)
	stop(sQuote("eps.correct"), " must be 0, 1, or 2")
