    ...
    for (k in 1:nMerge) {
        ...
        if (all(neg)) { ##two leaves
            ...
            attr(zk, "members") <- attr(z[[X[1 + isL]]], "members") +
                one
            attr(zk, "midpoint") <- (1 + attr(z[[X[1 + isL]]],
                "midpoint"))/2
            ...
            attr(zk, "midpoint") <- (attr(z[[x[1]]], "members") +
                attr(z[[x[1]]], "midpoint") + attr(z[[x[2]]],
                "midpoint"))/2
        }
          ...
          nest.lev = 0, indent.str = "", ...)
{
    ...
    if(!isLeaf(object)) {
        ...
        at <- at[!(names(at) %in% c("class", "height", "leaf",
                                    "members"))]
        at$label <- NULL
...
plot.dendrogram <-
  function (x, type = c("rectangle", "triangle"), center = FALSE,
    ...
    xaxt="n", yaxt="s",
    edgePar = list(), leaflab = c("perpendicular", "textlike",
        "none"), xlab = "", ylab = "", horiz = FALSE, ...)
{
    type <- match.arg(type)
    ...
    plot(0, xlim = xlim, ylim = ylim, type = "n", xlab = xlab,  xaxt=xaxt,
        yaxt = yaxt, ylab = ylab, ...)
    if (edge.root) {
...
plotNode <- function (x1, x2, subtree, type, center, leaflab, nodePar, edgePar,
    horiz = FALSE)
{
    ...
    Xtract <- function(nam, L, default, indx) rep(if (any(nam ==
        names(L)))
        L[[nam]]
    else default, length = indx)[indx]
    if (!is.null(nPar)) {
        i <- if (inner || hasP)
            1
        else 2
    ...
    if (isLeaf(subtree)) {
        if (leaflab == "perpendicular") {
            ...
            text(X, Y, attr(subtree,"label"), xpd = TRUE, srt = srt,
               adj = adj)
        }
    }
    else if (inner) {
        for (k in 1:length(subtree)) {
            ...
            vln <- NULL
            if ((isLeaf(child) && leaflab == "textlike") || !is.null(attr(child,
                "text"))) {
                nodeText <- if (isLeaf(child))
                  attr(child, "label")
                else as.character(attr(child, "text"))
                if (getOption("verbose"))
                  cat("-- with \"text\"", nodeText)
            ...
            if (!is.null(attr(child, "edgetext"))) {
                ...
                if (type == "triangle") {
                  polygon(c(mx - hlm, mx - 1.3 * hlm, mx - hlm,
                    mx + hlm, mx + 1.3 * hlm, mx + hlm), c(my -
                    vlm, my, my + vlm, my + vlm, my, my - vlm),
                    col = "white")
                  text(mx, my, edgeText)
                }
                else {
                  polygon(c(xBot - hlm, xBot - 1.3 * hlm, xBot -
                    hlm, xBot + hlm, xBot + 1.3 * hlm, xBot +
                    hlm), c(my - vlm, my, my + vlm, my + vlm,
                    my, my - vlm), col = "white")
