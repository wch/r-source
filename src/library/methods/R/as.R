...
coerce <-
  ## Coerce `object' to be of the same class as `to'.  Not a function you should usually
...
setAs <- 
  function(from, to, def, where = 1)
  {
    ...
    if(extends(from, to, TRUE)) {
      ...
      if(!identical(formalArgs(def), "object"))
        stop("Invalid function definition:  should be a function of one argument, named \"object\"")
      formals(def, envir = environment(def)) <- formals(function(object, to)NULL)
