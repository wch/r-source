...
isGeneric <-
  ...
  if(missing(fdef)) {
    if(missing(where)) {
      ...
      gotIt <- (if(is.environment(where)) exists(f, envir=where, inherits=F) else exists(f, where, inherits=F))
      if(!gotIt)
        return(FALSE)
...
selectMethod <-
  ...
  function(f, signature = character(), optional = FALSE, inherited = T, mlist) {
    if(missing(mlist)) {
      mlist <- getMethods(f)
      if(is.null(mlist)) {
        if(optional)
...
removeMethods <-
  ...
  function(f, where) {
    ...
    if(!isGeneric(f))
      return(FALSE)
    default <- getMethod(f, optional=T)
