...
@
It is possible to include non-finite values,
{\tt NA}, {\tt NaN}, {\tt Inf}, and {\tt -Inf},
in specifications of locations and sizes in
\grid{} functions.  This document describes how
\grid{} responds to non-finite values in different cases.
...
grid.text(c("segments", "text", "lines", "rectangles",
  "circles", "points"),
...
by the {\tt id} argument.  All polygons with the same {\tt id}
receive the same {\tt gp} settings.
