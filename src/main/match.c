...
patchArgsByActuals(SEXP formals, SEXP supplied, SEXP cloenv)
{
    ...
    for(b = supplied, a = prsupplied; ! IS_R_NilValue(b); b = CDR(b), a = CDR(a)) {
        SETCAR(a, CAR(b));
        SET_ARGUSED(a, 0);
        SET_TAG(a, TAG(b));
    }
    ...
    farg_i = 0;
    while (! IS_R_NilValue(f)) {
	if (! IS_R_NilValue(TAG(f))) {
	    for (b = prsupplied; ! IS_R_NilValue(b); b = CDR(b)) {
	...
	if (farg[farg_i] == FS_UNMATCHED) {
		...
		for (b = prsupplied; ! IS_R_NilValue(b); b = CDR(b)) {
		    if (!ARGUSED(b) && ! IS_R_NilValue(TAG(b)) &&
			pmatch(TAG(f), TAG(b), seendots)) {
    ...
    b = prsupplied;
    farg_i = 0;
    while (! IS_R_NilValue(f) && ! IS_R_NilValue(b)) {
	if (SEXP_EQL(TAG(f), R_DotsSymbol)) {
	    /* Done, ... and following args cannot be patched */
	    break;
	} else if (farg[farg_i] == FS_MATCHED_PRESENT) {
	    /* Note that this check corresponds to IS_R_MissingArg(CAR(b)) */
	    /* in matchArgs */
