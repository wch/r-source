...
SEXP attribute_hidden do_sprintf(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	*outputString;
    ...
    /* It there is only one format string used multiple times,
       do the coercion in advance */
    if (nfmt == 1  && maxlen > 1) {
	need_coercion = FALSE;
	/* now do any necessary coercions */
	formatString = translateChar(STRING_ELT(format, 0));
    ...
    /* Here we do the format analysis a row at a time */
	...
	for (cur = 0, cnt = 0; cur < n; cur += chunk) {
		...
		if (cur < n - 1 && formatString[cur + 1] == '%') {
			...
			_this = a[nstar];
			if(need_coercion && TYPEOF(_this) == REALSXP)
			    _this = coerceVector(_this, INTSXP);
			if(TYPEOF(_this) != INTSXP || LENGTH(_this)<1 ||
