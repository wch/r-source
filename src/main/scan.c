...
static void extractItem(char *buffer, SEXP ans, int i)
{
	...
	if (isNAstring(buffer, 0))
	    ...
	    INTEGER(ans)[i] = Strtoi(buffer, &endp, 10);
...
SEXP do_typecvt(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	res = Strtoi(tmp, &endp, 10); if (res == NA_INTEGER) isinteger = FALSE;
	...
	for (i = 0; i < len; i++) {
		...
		INTEGER(rval)[i] = Strtoi(tmp, &endp, 10);
