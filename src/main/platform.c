...
SEXP attribute_hidden do_fileshow(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	if (!isNull(el) && ! IS_NA_STRING(el))
	    ...
	    f[i] = acopy_string(translateChar(el));
            ...
            error(_("invalid filename specification"));
	if (! IS_NA_STRING(STRING_ELT(hd, i)))
...
SEXP attribute_hidden do_fileappend(SEXP call, SEXP op, SEXP args, SEXP rho)
{
	...
	for (int i = 0; i < n; i++) {
	    if (IS_R_NilValue(STRING_ELT(f1, i%n1)) ||
		IS_R_NilValue(STRING_ELT(f2, i%n2)))
		LOGICAL(ans)[i] = 0;
	    else
		LOGICAL(ans)[i] =
...
SEXP attribute_hidden do_filecreate(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n; i++) {
	LOGICAL(ans)[i] = 0;
	if (IS_NA_STRING(STRING_ELT(fn, i))) continue;
...
SEXP attribute_hidden do_fileremove(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    PROTECT(ans = allocVector(LGLSXP, n));
    for (i = 0; i < n; i++) {
	if (! IS_NA_STRING(STRING_ELT(f, i))) {
	    LOGICAL(ans)[i] =
#ifdef Win32
...
SEXP attribute_hidden do_filesymlink(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    PROTECT(ans = allocVector(LGLSXP, n));
    for (i = 0; i < n; i++) {
	if (IS_NA_STRING(STRING_ELT(f1, i%n1)) ||
	    IS_NA_STRING(STRING_ELT(f2, i%n2)))
	    LOGICAL(ans)[i] = 0;
	else {
#ifdef Win32
...
SEXP attribute_hidden do_filerename(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < n1; i++) {
	if (IS_NA_STRING(STRING_ELT(f1, i)) ||
	    IS_NA_STRING(STRING_ELT(f2, i))) {
	    LOGICAL(ans)[i] = 0;
	    continue;
	}
#ifdef Win32
...
SEXP attribute_hidden do_listfiles(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    if (isString(p) && length(p) >= 1 && ! IS_NA_STRING(STRING_ELT(p, 0)))
	pattern = TRUE;
	...
	if (IS_NA_STRING(STRING_ELT(d, i))) continue;
...
SEXP attribute_hidden do_fileexists(SEXP call, SEXP op, SEXP args, SEXP rho)
{
    ...
    for (i = 0; i < nfile; i++) {
	LOGICAL(ans)[i] = 0;
	if (! IS_NA_STRING(STRING_ELT(file, i))) {
#ifdef Win32
...
SEXP attribute_hidden do_dircreate(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	error(_("invalid '%s' argument"), "path");
    if (IS_NA_STRING(STRING_ELT(path, 0))) return ScalarLogical(FALSE);
    show = asLogical(CADR(args));
    if (show == NA_LOGICAL) show = 0;
...
#endif
	if (! IS_NA_STRING(STRING_ELT(paths, i))) {
#ifdef Win32
