...
Rboolean R_cycle_detected(SEXP s, SEXP child) {
    ...
    if (isPairList(child)) {
	    ...
	    if (SEXP_EQL(s, el) || R_cycle_detected(s, CAR(el)))
                return TRUE;
	    if (! IS_R_NilValue(ATTRIB(el)) && R_cycle_detected(s, ATTRIB(el)))
		return TRUE;		
	    el = CDR(el);
	}
...
static R_INLINE SEXP duplicate_list(SEXP s, Rboolean deep)
{
    ...
    val = R_NilValue;
    for (sp = s; ! IS_R_NilValue(sp); sp = CDR(sp))
	val = CONS(R_NilValue, val);

    PROTECT(val);
    for (sp = s, vp = val; ! IS_R_NilValue(sp); sp = CDR(sp), vp = CDR(vp)) {
